  ...
  return (
    <>
      ...
      <PageHeader
        variant="desk"
        title="Suvidha Desk"
        description={service ? service.title : "Pick a service to start"}
        actions={
        ...
        <div className="flex flex-col gap-4">
          <div className="desk-notice flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-foreground">
              <span className="font-semibold text-primary">Judge path · </span>
              Sanika RTO fixtures → verify → pack
            </p>
            <Button
              disabled={busy}
              onClick={() => void runJudgeDemo()}
                ...
                {group.items.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-3 text-left transition-colors duration-200",
                        on
                          ? "bg-secondary"
                    ...
                    >
                      ...
                      <p className="shrink-0 text-[11px] text-muted-foreground">
                        {s.required_docs.length} docs · {s.form_fields.length} fields
                      </p>
                    </button>
                  );
                })}
